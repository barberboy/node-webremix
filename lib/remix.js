...
var checkRemixes = function (media, remix, options, callback) {
  var dimensions = { width: options.width || DEFAULT_VIDEO_WIDTH, height: options.height || DEFAULT_VIDEO_HEIGHT };
  remix = matchYoutube.process(media, remix, dimensions);
  remix = matchRdio.process(media, remix, dimensions);
  remix = matchVimeo.process(media, remix, dimensions);
  remix = matchImage.process(media, remix, dimensions);
  remix = matchInstagram.process(media, remix, dimensions);

  if (!remix.isMatched) {
    matchLink.process(media, remix, dimensions, function(err, result) {
      if (err) {
        callback(err);
      } else {
